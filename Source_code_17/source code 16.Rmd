...
cytokine_chemokine_receptor_list <-
  biomaRt::getBM(
    filters = "go",
...
```{r}
#cytokine_chemokine_receptor list
drg_cytokine_chemokine_receptor=c()
for(i in c(cytokine_chemokine_receptor_list$ensembl_gene_id))
  if (i %in% row.names(drg_cluster))
    drg_cytokine_chemokine_receptor=c(drg_cytokine_chemokine_receptor,i)

```

```{r}
drg_cluster <- drg_cluster[drg_cytokine_chemokine_receptor,]

```

```{r}
# Get cytokine_chemokine_receptor names from Ensembl IDs
cytokine_chemokine_receptor <-
  biomaRt::getBM(
...
drg_cytokine_chemokine_receptor_heatmap <-
  drg_cluster %>%
  left_join(cytokine_chemokine_receptor, by = c("Ensemble_ID" = "ensembl_gene_id")) %>%
...
```{r}
# Add gene names, filter and create Seurat object
drg_cytokine_chemokine_receptor_heatmap <- drg_cytokine_chemokine_receptor_heatmap[drg_cytokine_chemokine_receptor_heatmap$all_cluster>0,]
```

```{r}
drg_cytokine_chemokine_receptor_heatmap_ranked <- drg_cytokine_chemokine_receptor_heatmap[order(-drg_cytokine_chemokine_receptor_heatmap$all_cluster),]
drg_cytokine_chemokine_receptor_heatmap_ranked
```

```{r}
write.csv(drg_cytokine_chemokine_receptor_heatmap_ranked, "C:/E/Posdoc work at Upenn/Lab_work/Projected_I_am_intersted_in/Transcriptome of human DRG neurons/Final analysis for publication/Drug target heat map/cytokine_chemokine_receptor.csv")
```


```{r, fig.width=8, fig.height=30}
drg_cytokine_chemokine_receptor_heatmap_ranked_matrix <-data.matrix(drg_cytokine_chemokine_receptor_heatmap_ranked[,])

lmat = rbind(c(0,4),c(3,1),c(0,2))
lwid = c(1,5)
lhei = c(0.1,2.5,0.5)

heatmap.2(drg_cytokine_chemokine_receptor_heatmap_ranked_matrix, dendrogram = 'none', Colv = FALSE, Rowv = FALSE, scale = "none", col = bluered(100), trace = "none", density.info = "none",  keysize=0.05, key.title=NA, lmat = lmat, lwid = lwid, lhei = lhei)
...
drg_cytokine_chemokine_receptor_heatmap_ranked_matrix = subset(drg_cytokine_chemokine_receptor_heatmap_ranked_matrix, select = -c(NP) )
...
heatmap.2(drg_cytokine_chemokine_receptor_heatmap_ranked_matrix, dendrogram = 'none', Colv = FALSE, Rowv = FALSE, scale = "none", col = bluered(100), trace = "none", density.info = "none",  keysize=0.2, key.title=NA, lmat = lmat, lwid = lwid, lhei = lhei, cexRow=2, cexCol=2.7, margins=c(1,8)) 
...
drg_cytokine_chemokine_receptor_heatmap_cluster1_ranked <- drg_cytokine_chemokine_receptor_heatmap[order(-drg_cytokine_chemokine_receptor_heatmap$NP),]
#drg_GPCR_heatmap_cluster1_ranked

drg_cytokine_chemokine_receptor_heatmap_cluster1_ranked_matrix <-data.matrix(drg_cytokine_chemokine_receptor_heatmap_cluster1_ranked[,])

lmat = rbind(c(0,4),c(3,1),c(0,2))
lwid = c(1,5)
lhei = c(0.2,5,0.5)

heatmap.2(drg_cytokine_chemokine_receptor_heatmap_cluster1_ranked_matrix, dendrogram = 'none', Colv = FALSE, Rowv = FALSE, scale = "none", col = bluered(100), trace = "none", density.info = "none",  keysize=0.05, key.title=NA, lmat = lmat, lwid = lwid, lhei = lhei)

```

```{r, fig.width=10, fig.height=30}
drg_cytokine_chemokine_receptor_heatmap_cluster1_ranked <- drg_cytokine_chemokine_receptor_heatmap[order(-drg_cytokine_chemokine_receptor_heatmap$NP1),]
#drg_GPCR_heatmap_cluster1_ranked
...
drg_cytokine_chemokine_receptor_heatmap_ranked_row_normalization <- t(apply(drg_cytokine_chemokine_receptor_heatmap_ranked, 1, divide_by_max))



drg_cytokine_chemokine_receptor_heatmap_ranked_matrix_row_normalization <-data.matrix(drg_cytokine_chemokine_receptor_heatmap_ranked_row_normalization[,-(1:2)])
...
heatmap.2(drg_cytokine_chemokine_receptor_heatmap_ranked_matrix_row_normalization, dendrogram = 'none', Colv = FALSE, Rowv = FALSE, scale = "none", col = bluered(100), trace = "none", density.info = "none",  keysize=0.05, key.title=NA, lmat = lmat, lwid = lwid, lhei = lhei,cexRow = 1)
