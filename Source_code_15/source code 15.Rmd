...
#ion_channels list
drg_ion_channels=c()
for(i in c(ion_channels_list$ensembl_gene_id))
  if (i %in% row.names(drg_cluster))
    drg_ion_channels=c(drg_ion_channels,i)

```

```{r}
drg_cluster <- drg_cluster[drg_ion_channels,]

```

```{r}
# Get ion_channels names from Ensembl IDs
ion_channels <-
  biomaRt::getBM(
...
drg_ion_channels_heatmap <-
  drg_cluster %>%
  left_join(ion_channels, by = c("Ensemble_ID" = "ensembl_gene_id")) %>%
...
```{r}
# Add gene names, filter and create Seurat object
drg_ion_channels_heatmap <- drg_ion_channels_heatmap[drg_ion_channels_heatmap$all_cluster>0,]
```

```{r}
drg_ion_channels_heatmap_ranked <- drg_ion_channels_heatmap[order(-drg_ion_channels_heatmap$all_cluster),]
drg_ion_channels_heatmap_ranked
```

```{r}
write.csv(drg_ion_channels_heatmap_ranked, "C:/E/Posdoc work at Upenn/Lab_work/Projected_I_am_intersted_in/Transcriptome of human DRG neurons/Final analysis for publication/Drug target heat map/ion_channels.csv")
```


```{r, fig.width=8, fig.height=30}
drg_ion_channels_heatmap_ranked_matrix <-data.matrix(drg_ion_channels_heatmap_ranked[,])

lmat = rbind(c(0,4),c(3,1),c(0,2))
lwid = c(1,5)
lhei = c(0.1,2.5,0.5)

heatmap.2(drg_ion_channels_heatmap_ranked_matrix, dendrogram = 'none', Colv = FALSE, Rowv = FALSE, scale = "none", col = bluered(100), trace = "none", density.info = "none",  keysize=0.05, key.title=NA, lmat = lmat, lwid = lwid, lhei = lhei)
...
```{r, fig.width=8, fig.height=30}
drg_ion_channels_heatmap_ranked_matrix <-data.matrix(drg_ion_channels_heatmap_ranked[,])
drg_ion_channels_heatmap_ranked_matrix = subset(drg_ion_channels_heatmap_ranked_matrix, select = -c(NP) )
...
heatmap.2(drg_ion_channels_heatmap_ranked_matrix, dendrogram = 'none', Colv = FALSE, Rowv = FALSE, scale = "none", col = bluered(100), trace = "none", density.info = "none",  keysize=0.05, key.title=NA, lmat = lmat, lwid = lwid, lhei = lhei, cexRow=1, cexCol=2.7, margins=c(1,8))
...
drg_ion_channels_heatmap_cluster1_ranked <- drg_ion_channels_heatmap[order(-drg_ion_channels_heatmap$PEP.H8),]
#drg_GPCR_heatmap_cluster1_ranked

drg_ion_channels_heatmap_cluster1_ranked_matrix <-data.matrix(drg_ion_channels_heatmap_cluster1_ranked[,])

lmat = rbind(c(0,4),c(3,1),c(0,2))
lwid = c(1,5)
lhei = c(0.2,5,0.5)

heatmap.2(drg_ion_channels_heatmap_cluster1_ranked_matrix, dendrogram = 'none', Colv = FALSE, Rowv = FALSE, scale = "none", col = bluered(100), trace = "none", density.info = "none",  keysize=0.05, key.title=NA, lmat = lmat, lwid = lwid, lhei = lhei, cexRow=1, cexCol=2.7, margins=c(1,8))

```

```{r, fig.width=10, fig.height=30}
drg_ion_channels_heatmap_cluster1_ranked <- drg_ion_channels_heatmap[order(-drg_ion_channels_heatmap$NP1),]
#drg_GPCR_heatmap_cluster1_ranked
...
heatmap.2(drg_ion_channels_heatmap_cluster1_ranked_matrix, dendrogram = 'none', Colv = FALSE, Rowv = FALSE, scale = "none", col = bluered(100), trace = "none", density.info = "none",  keysize=0.05, key.title=NA, lmat = lmat, lwid = lwid, lhei = lhei)
...
drg_ion_channels_heatmap_ranked_row_normalization <- t(apply(drg_ion_channels_heatmap_ranked, 1, divide_by_max))



drg_ion_channels_heatmap_ranked_matrix_row_normalization <-data.matrix(drg_ion_channels_heatmap_ranked_row_normalization[0:49,-(1:2)])
...
heatmap.2(drg_ion_channels_heatmap_ranked_matrix_row_normalization, dendrogram = 'none', Colv = FALSE, Rowv = FALSE, scale = "none", col = bluered(100), trace = "none", density.info = "none",  keysize=0.05, key.title=NA, lmat = lmat, lwid = lwid, lhei = lhei,cexRow = 1)
...
drg_ion_channels_heatmap_ranked_matrix_row_normalization <-data.matrix(drg_ion_channels_heatmap_ranked_row_normalization[0:99,-(1:2)])
